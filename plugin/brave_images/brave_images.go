...
type (
	...
	Service interface {
		GetImages(query string) (model.ImageSearchImages, error)
		GetImagesFromQueryID(queryID int64) (model.ImageSearchImages, error)
		SaveImages(query string, wrapper *model.ImageSearchImages) (int64, error)
...
func (p *Plugin) doImageSearch(b *gotgbot.Bot, c *plugin.GobotContext) error {
	...
	var wrapper model.ImageSearchImages
	var err error
...
func (p *Plugin) onImageSearch(b *gotgbot.Bot, c plugin.GobotContext) error {
	...
	if err != nil {
		...
		} else if httpError, ok := errors.AsType[*httpUtils.HttpError](err); ok && httpError.StatusCode == http.StatusUnprocessableEntity {
